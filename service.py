...
class DBUtil():
    ...
    def CreateList(self, uplistid, listname, is_list, username):
        ...
        # 查询给的目录id是否存在，若存在直接返回目录id，若不存在则新建
        sql3 = 'SELECT SKT32.SKF413 FROM skt32 WHERE skf414 = %s AND SKT32.skf415 = %s'
        try:
            self.cur.execute(sql3,[listname,uplistid])
            results = self.cur.fetchone()
            if results is not None:
                row1 = results[0]
                return row1
                ...
                # 调用三个存储过程
                ListURL1 = "http://127.0.0.1:8080/ckapi/api/1/v2/SKupdate_list_id.jsp"
                ListURL2 = "http://127.0.0.1:8080/ckapi/api/1/v2/SKupdate_power.jsp"
                ListURL3 = "http://127.0.0.1:8080/ckapi/api/1/v2/SKupdate_mylist.jsp"
    ...
    # 写入借阅信息
    def BorrowInfo(self, borrowingNum, documentNumber, reasonApplication, userID, startTime, overTime, preview, download, print,
                   approvalStatus, Approve_time, Approve_name, Approver, Approval_operation, Approval_comments, is_document,lend):
        ...
        is_work = False
        try:
            self.cur.execute(sql,(borrowingNum, documentNumber, reasonApplication, userID, startTime, overTime,preview, download, print, approvalStatus, lend, Approve_name, Approver,Approval_operation,Approval_comments,Approve_time ))
            is_work = True
            # 提交
            self.conn.commit()
        except Exception as e:
    ...
    # 档案下载获取url
    def documentDownloadUrl(self,documentNumber, userID):
        ...
        # 获取附件编号
        Annex_SQL = "SELECT skf376 FROM skt28 WHERE skf906='%s'" % documentNumber
        self.cur.execute(Annex_SQL)  # 获取附件编号
        result = self.cur.fetchone()  # 取所有结果集
        Annex = result[0]
        ...
        finalUrl = url + encryptByMD5(BeforeMD5) + '&time=30&isShowPrint=0&isShowLoadFile=1&fileids=' + Annex + '&waterMarkName=&isCopy=0&isShowPreview=0&isShowTop=0&isDirectPreview=0'
        # print(finalUrl)

        return finalUrl

    # 档案打印获取url
    def documentPrintUrl(self,documentNumber, userID):
        # 获取服务器地址
        self.cur.execute("select skf991,skf689 from skt57")  # 获取服务器地址和单位号
        result = self.cur.fetchone()  # 取所有结果集
        weburl = result[0]  # 服务器地址
        ...
        finalUrl = url + encryptByMD5(BeforeMD5) + '&time=30&isShowPrint=1&isShowLoadFile=0&fileids=' + Annex + '&waterMarkName=&isCopy=0&isShowPreview=0&isShowTop=0&isDirectPreview=1'
        # print(finalUrl)
...
def documentBorrow(preview:Preview):
    ...
    if preview.approvalStatus == "3":
        # 区分电子档案与实体档案：电子档案为1，实体档案为2，电子和实体档案为3
        if preview.is_document == "1" or preview.is_document == '3':
            if dbutil.ConnectTODB():
                finalUrl = dbutil.BorrowInfo(preview.borrowingNum, preview.documentNumber, preview.reasonApplication, preview.userID, preview.startTime, preview.overTime, preview.preview,
                                  preview.download,
                                  preview.print, preview.approvalStatus, preview.Approve_time, preview.Approve_name, preview.Approver, preview.Approval_operation,
                                  preview.Approval_comments, preview.is_document,preview.lend)

                # print("链接数据库")
                results={"code":0,"data":[{"success":finalUrl}],"msg":"成功"}
            ...
            results = {"code": 2, "data": "档案为实体档案，到档案室借阅", "msg": "成功"}
            return results
...
def getPreviewUlr(documentNumber:str, userID:str):
    dbutil = DBUtil()
    # print(preview.borrowingNum)
    # print(type(is_document))
    if dbutil.ConnectTODB():
...
def getDownload(documentNumber:str, userID:str):

    dbutil = DBUtil()
    if dbutil.ConnectTODB():
        finalUrl = dbutil.documentDownloadUrl(documentNumber, userID)
        # print("链接数据库")
        # results = {"code": 0, "data": [{"url": finalUrl}], "msg": "成功"}
        return RedirectResponse(finalUrl)
        # return results
...
def documentPrint(documentNumber:str, userID:str):
    dbutil = DBUtil()

    if dbutil.ConnectTODB():
        finalUrl = dbutil.documentPrintUrl(documentNumber, userID)
        # print("链接数据库")
        # results = {"code": 0, "data": [{"url": finalUrl}], "msg": "成功"}
        return RedirectResponse(finalUrl)
        # return results
...
def setSecret(secret:Secret):
    ...
    if dbutil.ConnectTODB():
        ...
        if is_work:
            results = {"code": 1, "data": "修改成功", "msg": "成功"}
            return results
        else:
            results = {"code": 0, "data": "修改失败", "msg": "失败"}
            return results
...
def documentDestroy(destroy:Destroy):
    ...
    if dbutil.ConnectTODB():
        ...
        # 查看返回值是否正确，如果正确，说明数据库已修改
        if is_work:
            results = {"code": 1, "data": "删除成功", "msg": "成功"}
            return results
        else:
            results = {"code": 0, "data": "删除失败", "msg": "失败"}
            return results
...
def createList(uplistid:int, username:str,listname:str,is_list:int):
    ...
    if dbutil.ConnectTODB():
        ...
        # 查看返回值是否正确，如果正确，说明数据库已修改
        if Listid:
            results = {"code": 1, "Listid": Listid, "msg": "成功"}
            return results
        else:
            results = {"code": 0, "data": "新建目录失败", "msg": "失败"}
            return results
...
def secretPower(searchid:str,username:str):
    ...
    if dbutil.ConnectTODB():
        ...
        results = {"code": 0, "data": "数据库连接失败", "msg": "失败"}
        return results
